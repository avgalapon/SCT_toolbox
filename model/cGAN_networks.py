...
###############################################################################
# Helper Functions - model architecture
###############################################################################
...
def get_scheduler(optimizer, lp):
    ...
    """
    if lp['lr_policy'] == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + lp['epoch_start'] - lp['epochs']) / float(lp['decayLR'] + 1)
            return lr_l
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule)
    else:
        return NotImplementedError('learning rate policy [%s] is not implemented', lp['lr_policy'])
    return scheduler
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        self.output_layer = nn.Conv2d(ngf, output_nc, kernel_size=1, padding=0)
        self.std_layer = nn.Conv2d(ngf, output_nc, kernel_size=1, padding=0)
